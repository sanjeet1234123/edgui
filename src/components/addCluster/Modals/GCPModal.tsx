...
import type { GCPClusterConfiguration } from '@/types/addClusterType'
...
const gcpClusterConfigurationSchema = z.object({
  clusterName: z.string().min(1, 'Cluster name is required'),
})

function GCPModal({ opened, close }: GCPModalProps) {
  const navigate = useNavigate()
  // Upload cluster file mutation
  const { mutate: gcpOnboard, isPending } = useGCPOnboardMutation()
  ...
  const form = useForm<GCPClusterConfiguration>({
    initialValues: {
      clusterName: '',
    },
    validate: zodResolver(gcpClusterConfigurationSchema),
  })
    ...
    gcpOnboard(
      {
        ...
        credentials_file: fileData.files[0],
      },
      {
        onSuccess: () => {
          stack.open('success')
          ...
          <Group>
            ...
            <Stack gap={0}>
              ...
              <Text fz="var(--size-sm)" fw={400} c="dimmed">
                Please configure and add your GCP account
              </Text>
            </Stack>
          </Group>
        }
        ...
        <form onSubmit={form.onSubmit(handleSubmit)}>
          <Stack gap="xl">
            ...
            <Stack align="center" gap={0}>
              ...
              <Text fz="var(--size-sm)" fw={400} c="dimmed">
                Please provide the following details for your GCP cluster
              </Text>
            </Stack>
        ...
        title={
          <Group>
            ...
            <Stack gap={0}>
              ...
              <Text fz="var(--size-sm)" fw={400} c="dimmed">
                Your GCP cluster has been added successfully
              </Text>
            </Stack>
          </Group>
        }
      >
        <Stack gap="xl" align="center">
          ...
          <Text ta="center" fz="var(--size-lg)" fw={500}>
            Your GCP account has been successfully onboarded. You're all set to
            start deploying, managing resources, and exploring powerful cloud
            capabilities.
          </Text>
